...
// Getters
int const Map::get_width()  const  { return m_width;  }
int const Map::get_height() const  { return m_height; }

unsigned int* const Map::get_level_data() const { return m_level_data; }
GLuint        const Map::get_texture_id() const { return m_texture_id; }

float const Map::get_tile_size()    const { return m_tile_size;    }
int   const Map::get_tile_count_x() const { return m_tile_count_x; }
int   const Map::get_tile_count_y() const { return m_tile_count_y; }

std::vector<float> const Map::get_vertices()            const { return m_vertices;             }
std::vector<float> const Map::get_texture_coordinates() const { return m_texture_coordinates; }

float const Map::get_left_bound()   const { return m_left_bound;   }
float const Map::get_right_bound()  const { return m_right_bound;  }
float const Map::get_top_bound()    const { return m_top_bound;    }
float const Map::get_bottom_bound() const { return m_bottom_bound; }
